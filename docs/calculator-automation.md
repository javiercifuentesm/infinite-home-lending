...
### Final acceptance and reply gate

The operator inbox check accepts an optional third argument `es` to exercise a separate idempotent Spanish fixture for the designated test recipient. It must fail if actual report delivery is not confirmed; provider acceptance alone is insufficient. Inspect the existing message receipt before any retry; do not create another fixture just to resend an accepted message.

Automatic nurture requires both CALCULATOR_NURTURE_ENABLED=true and CALCULATOR_REPLY_TRACKING_VERIFIED=true. Leave the latter unset until an actual incoming borrower reply demonstrably reaches the CRM and changes a signal observed by the worker. A connected inbox alone is insufficient. HubSpot's reply-only logging generally requires the original thread to have been logged from the CRM or the connected mail client. Its all-emails-from-known-contacts option can log incoming messages from existing contacts when the advisor work inbox is connected; verify the actual account setting and end-to-end signal. See https://knowledge.hubspot.com/connected-email/manage-contact-email-logging-rules and https://knowledge.hubspot.com/connected-email/log-email-replies-in-the-crm. Brevo inbound parsing is a separate option requiring receiving-subdomain DNS delegation and a secured webhook; it is not enabled here.

The fixture-only capture harness at scripts/qa/calculator-report-harness.tsx imports the actual form and language provider and mocks network writes. It supports language switching, simulated failure/retry and displays a safe receipt for consent, UUID and retry identity checks. It is test code, not an application route. Cloud browser policy currently prevents opening its local HTML page, so responsive capture acceptance remains outstanding; do not label a source review or mocked HTTP test as a completed phone/browser test.
