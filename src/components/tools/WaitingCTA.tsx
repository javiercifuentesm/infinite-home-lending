...
export function WaitingCTA() {
  ...
  return (
    <div className="flex flex-col gap-6 rounded-xl border border-[var(--tcw-border,#e2e8f0)] bg-[var(--tcw-surface,#fff)] p-6 sm:flex-row sm:items-center sm:justify-between sm:gap-8 sm:p-8">
      <div>
        <p className="text-[13px] font-medium text-[var(--tcw-text-primary,#0B2A4A)]">{t("tool.waiting.cta.title")}</p>
        <p className="mt-1 text-[11px] text-[var(--tcw-text-muted,#64748b)]">{t("tool.waiting.cta.sub")}</p>
      </div>
      <Link
        to="/contact"
