import {
  formatCompactWaitI18n,
  formatWaitPeriodLabelI18n,
    ...
    <div className="rounded-xl border border-[var(--tcw-border,#e2e8f0)] bg-[var(--tcw-surface,#fff)] p-5 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <span
          ...
          aria-live="polite"
        >
          {formatWaitPeriodLabelI18n(safe, t)}
        </span>
      </div>
      <div className="mt-4 flex flex-wrap items-baseline gap-2">
        <span className="font-[Georgia,serif] text-2xl font-semibold text-[#0B2A4A]">
          {formatCompactWaitI18n(safe, t)}
        </span>
        <span className="text-[13px] text-[var(--tcw-text-muted,#64748b)]">{t("tool.waiting.slider.selected")}</span>
      </div>
      <input
        type="range"
        min={1}
